...
updated_at: "2026-09-26T13:46:34.403Z"
---
...
원하는 등록 축제를 찾아 시군구 방문 흐름·관광자원·개최 시기를 공통 화면에서 이용할 수 있다. 공식 API 전국 방문·등록 일정 보관과 일별 갱신, 검토된 기존 회차 비교를 유지한다. 기존·새 축제 모두 관광지·문화시설·음식점·숙박을 선택하고 같은 방식으로 장소 소개·지도·직선거리를 살펴볼 수 있다. 기본 선택은 관광지·문화시설이고 기록은 선택이다. 설계 후 실제 Opus5.5 네 작업 구현 위임·직접 검수, 단위368·headless24·ARC·공개 실제 응답/두 흐름·업무 보존 확인을 완료했다. D:\download\project\pickDday\index.html의 최신 관광자원 묶음에서 기본 배치·실제 화면을 먼저 확인한다.
...
- 최종 시각 디자인·실제 담당자 관찰·전체 보조기술/200% 확대·개편35조회단위 공식 경계 교체·공동편집/공식승인·정형 API/Spec/Run 연결은 후속이다. 뉴스 AI요약은 나중에 검토한다. 현재 구현에 필요한 미결 사람 결정은 없다.
- 음식점·숙박 공통 연결은 사용 가능하다. 많은 장소가 겹친 지도에서 포인터로 개별 장소를 고르는 편의성, 운영시간·메뉴·객실·예약 가능 여부 구조화는 후속 검토다. 이번 구현을 막는 사람 결정은 없다.
