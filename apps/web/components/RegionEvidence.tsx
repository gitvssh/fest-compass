...
export function RegionEvidence() {
  ...
  return <div className="space-y-6"><header><p className="text-xs font-extrabold text-blue">지역 자료에서 올해 기획으로</p><h1 className="mt-2 text-3xl font-extrabold">담은 기획 근거</h1><p className="mt-3 text-sm leading-7 text-muted">선택 당시 값·조회 조건·누락·출처를 보관합니다. 원천이 바뀌어도 이 사본은 유지됩니다. 이 브라우저에만 저장되므로 파일로도 보관하세요.</p></header>
    ...
    <p className="text-sm text-muted">보관한 자료를 기획 후보의 아이템·장소·시기 판단에 연결할 수 있습니다. 예산 산출 연결은 다음 단계입니다.</p>
    ...
    {items.map(item => <article key={item.id} className="region-card space-y-3"><h2 className="text-xl font-extrabold">{item.title}</h2><Link className="region-primary no-print" href={`/planning/options?evidence=region:${item.id}`}>이 지역 근거로 후보 기획 →</Link><p className="text-sm text-muted">{item.result.region.provinceName} · {item.result.region.districtName} · 조회 기간 {item.result.query.start} ~ {item.result.query.end}</p><p className="text-xs text-muted">담은 시각: {new Date(item.savedAt).toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })}</p>
      ...
      {"resourceId" in item.selection && item.selection.boundary && <p className="break-words text-xs text-muted">선택 당시 참고 경계: SGIS {item.selection.boundary.boundaryDate} · 코드 연계 {item.selection.boundary.crosswalkDate} · 경계 코드 {item.selection.boundary.codes.join(", ")} · 자료 버전 {item.selection.boundary.version}. 현재 행정구역과 다를 수 있습니다. <a className="underline" href={item.selection.boundary.source} target="_blank" rel="noreferrer">경계 원본</a></p>}
    </article>)}
  </div>;
}
