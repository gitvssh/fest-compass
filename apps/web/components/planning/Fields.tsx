...
export function Field({ label, value, onChange, multiline = false, type = "text", max = 2000 }: { label: string; value: string; onChange: (v: string) => void; multiline?: boolean; type?: string; max?: number }) {
  return <label className="block min-w-0 space-y-2 text-sm font-bold"><span>{label}</span>{multiline ? <textarea aria-label={label} className="workspace-input min-h-24" maxLength={max} value={value} onChange={e => onChange(e.target.value)} /> : <input aria-label={label} className="workspace-input" type={type} maxLength={max} value={value} onChange={e => onChange(e.target.value)} />}</label>;
}
