...
export function VisitComparison({ editions, onSave, initialRange }: { editions: Edition[]; onSave?: (s: Selection) => void; initialRange?: { from: number; to: number } }) {
  ...
  return <section className="region-card space-y-4"><h2 className="text-xl font-extrabold">개최 전후 방문 추세 비교</h2><p className="text-sm leading-7 text-muted">같은 지역·지표·단위·측정 방법의 보관값을 공통 축으로 표시합니다. 행사장 입장객·순방문객 합계·축제 효과가 아닙니다. 다른 지역의 미확보 값을 대신 채우지 않습니다.</p>
    {!available.length ? <p className="rounded-xl bg-paper p-4">선택 회차의 비교 가능한 방문 이력이 미확보 상태입니다.</p> : <>
      ...
      <label className="no-print block text-sm font-bold">그래프 날짜 기준<select className="workspace-input mt-2 max-w-xs" value={axis} onChange={e=>setAxis(e.target.value)}><option value="relative">행사 시작일 기준 (D0)</option><option value="calendar">실제 날짜 기준</option></select></label>
