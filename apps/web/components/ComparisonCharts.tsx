...
export function CostCharts({ editions, onSave }: { editions: Edition[]; onSave?: (s: Selection)=>void }) {
  ...
  return <section className="region-card space-y-4"><h2 className="text-xl font-extrabold">비용 원문과 항목 구성</h2><p className="text-sm leading-7 text-muted">사업·범위·단계별 원문 금액입니다. 전체 축제 예산으로 통합하지 않습니다. 같은 문서의 계획·집행도 기재 금액을 보여주며 확정 예산 대비 효율로 해석하지 않습니다.</p>
    {editions.map(e=><div key={e.id} className="space-y-4 rounded-xl border border-ink/10 p-4"><h3 className="font-extrabold">{title(e)}</h3>{!e.costs.length?<p className="text-sm text-muted">비용 자료 미확보 · 0원으로 표시하지 않습니다.</p>:<>
      ...
      {e.costs.map(c=>{const parts=composition(c), expense=c.compositionKind==="expense", group=expense?"원가 항목":"재원";let running=0,colorIndex=0;const partColors=["#1649b8","#995008","#06736b","#7b3ca5","#b33d55"];const stops=parts?.map(p=>{const start=running;running+=p.share*100;const color=partColors[p.amount>0?colorIndex++%partColors.length:0];return `${color} ${start}% ${running}%`;});return <div key={c.id} className="space-y-3 border-t border-ink/10 pt-4"><h4 className="font-bold">{c.label}</h4><p className="text-xs leading-6">{c.year}년 · {c.stage} · 부가세 {c.vat}<br/>{c.scope}<br/>작성 부서·기관: {c.department}</p>
        {parts?<div className="grid items-center gap-4 sm:grid-cols-[180px_1fr]"><div role="img" aria-label={`${c.label} ${group} 구성: ${parts.map(p=>`${p.label} ${number(p.amount)}원 ${(p.share*100).toFixed(1)}%`).join(", ")}`} className="mx-auto flex h-40 w-40 items-center justify-center rounded-full" style={{background:`conic-gradient(${stops!.join(",")})`}}><div className="flex h-28 w-28 items-center justify-center rounded-full bg-white p-2 text-center text-xs font-bold">{number(c.amount)}원<br/>{expense?"해당 원가 합계":"해당 사업"}</div></div><div className="min-w-0 overflow-x-auto" tabIndex={0} aria-label={`${c.label} 구성 수치 표 · 좌우 이동 가능`}><table className="comparison-table"><caption className="sr-only">{c.label} {group}별 원금액과 구성비</caption><thead><tr><th>{group}</th><th>원</th><th>비율</th></tr></thead><tbody>{parts.map(p=><tr key={p.label}><th scope="row">{p.label}</th><td>{number(p.amount)}</td><td>{(p.share*100).toFixed(1)}%</td></tr>)}</tbody></table></div><p className="text-xs text-muted sm:col-span-2">분모 {number(c.amount)}원 · {group} 합계와 일치 · 소수 첫째 자리 반올림. 재원과 지출을 합친 구성이 아닙니다.</p></div>:<p className="rounded-xl bg-paper p-3 text-sm">전체 분모·중복 없는 항목 구성이 확인되지 않아 구성비는 그리지 않습니다.</p>}
