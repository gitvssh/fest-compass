...
export function FestivalComparison({ catalogue, initial, year }: { catalogue: Edition[]; initial: SearchContext; year: number }) {
  ...
  function changeMode(mode: SearchContext["mode"]){
    ...
    const next:SearchContext={...draft,mode,start:mode==="archive"?"2022-01-01":`${year}-01-01`,end:mode==="archive"?"2025-12-31":`${year}-12-31`,theme:"",dateRule:"overlap",queriedAt:null};setDraft(next);setApplied(next);
  }
