  ...
  return <div className="space-y-6">
    <header className="no-print space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button className={button} disabled={!workspace} onClick={() => download("pickDday-workspace.json", JSON.stringify(workspace, null, 2))}>작업 파일 내보내기</button>
    ...
    {corrupt && <div className={`${card} no-print`}><p>기존 저장 내용이 손상되어 덮어쓰기를 중지했습니다. 원본을 보관한 뒤 정상 작업 파일을 가져오세요.</p><button className={`${button} mt-3`} onClick={() => download("pickDday-recovery.txt", corrupt, "text/plain")}>손상된 원본 보관</button></div>}
