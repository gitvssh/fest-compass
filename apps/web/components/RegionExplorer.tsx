...
export function RegionExplorer({ year }: { year: number }) {
  ...
  return <div className="space-y-6">
    ...
    <nav aria-label="선택 지역 경로" className="flex flex-wrap items-center gap-2 rounded-2xl bg-white p-4 text-sm shadow-sm"><button className="font-bold text-blue underline" onClick={() => chooseProvince("")}>남한 전국</button><span aria-hidden="true">›</span>{province ? <button className="font-bold text-blue underline" onClick={() => chooseProvince(province)}>{label(province)}</button> : <span className="text-muted">시도 선택</span>}<span aria-hidden="true">›</span><span aria-current={district ? "location" : undefined}>{districtName || "시군구 선택"}</span>{home && <button className="region-button ml-auto" onClick={() => load(home.province, home.district)}>우리 지역 바로가기</button>}</nav>
    <div className="region-card grid grid-cols-2 gap-3 lg:hidden">
      <label className="text-sm font-bold">01 시도<select aria-label="시도 선택" className="workspace-input mt-2" value={province} onChange={e => chooseProvince(e.target.value)}><option value="">남한 전국</option>{provinces.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></label>
