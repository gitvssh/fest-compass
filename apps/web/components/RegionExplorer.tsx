  ...
  return <div className="space-y-6">
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[290px_minmax(0,1fr)]">
      ...
      <div className="min-w-0 space-y-4">
        ...
        </form><p className="mt-3 text-xs leading-5 text-muted">최대 366일. 축제·행사는 이 기간과 <strong>겹치는 일정의</strong> 등록 자료를 조회합니다. 관광지·문화시설·음식점·숙박은 현재 등록 목록이며 과거 영업·사용 가능 여부를 뜻하지 않습니다.</p></section>
        ...
        {data && <>
          ...
          <section className="region-card space-y-3" aria-label="조회 자료 목록"><div className="flex flex-wrap items-center justify-between gap-2"><h2 ref={listHeading} tabIndex={-1} className="text-xl font-extrabold">{districtName} · {TYPES[data.query.kind]}</h2><span className="region-tag">{data.resources.status === "unavailable" ? "조회 미확보" : `API 등록 ${data.resources.total}건`}</span></div>
            <p className="text-xs leading-6 text-muted">{data.resources.message} · 조회 {new Date(data.resources.collectedAt).toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })}<br />좌표 없는 자료 {resources.filter(r => r.longitude === null).length}건은 목록에 유지합니다.{bounds && " 공간 필터 적용 중 · 좌표 없는 자료는 포함합니다."}</p>
