...
/**
 * Provider introduction of one listed resource (same id + type + region), shared by both festival journeys.
...
/** Stable remount key: another region, type or id starts a fresh introduction (no earlier text or fold state). */
export const introKey = (region: RegionRef, item: ResourceItem) => `${region.code}-${item.kind}-${item.id}`;
