...
지역 공무원이 공공데이터로 지역 관광자원과 방문 흐름을 살펴 축제를 기획·개선하도록 돕는 웹 앱입니다. 기록 없이 자유롭게 탐색할 수 있고, 데이터가 목표 관광객이나 최적 개최일을 정하지 않습니다. 기록을 원하면 근거·가정·시나리오·결정·실측을 한 원장으로 연결합니다. 흥행 예측 서비스가 아닙니다.

화면에 보이는 이름은 `pickDday`입니다. npm 패키지 이름은 대문자를 쓸 수 없어 `pickdday`이며, 이미지 태그·DB 파일·브라우저 저장 키·저장 파일 형식 등 내부 식별자는 기존 `fest-compass` 값을 유지합니다.
