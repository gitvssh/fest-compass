...
try{
  ...
  await page.unroute("**/api/regions?*");await page.goto(`${base}/compare?province=44&district=230&mode=current&start=2026-01-01&end=2026-12-31`);await visible(page.getByText("관광정보 목록을 불러오지 못했어요. 잠시 후 다시 조회해 주세요.",{exact:false}));await page.getByRole("button",{name:"논산 3회차 방문 비교",exact:true}).click();await visible(page.getByText("52,671.5",{exact:true}));
