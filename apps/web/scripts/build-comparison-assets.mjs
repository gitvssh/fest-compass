...
const historyRaw = readFileSync(new URL("../data/regional-history-expanded.json", import.meta.url));
const expanded = JSON.parse(historyRaw), historyHash = createHash("sha256").update(historyRaw).digest("hex");
for (const e of editions.filter(e => e.festivalId === "imsil-cheese" || e.id === "baekje-gongju-2024")) {
  const d = expanded.datasets.find(d => d.region.code === `${e.region.province}${e.region.district}`);
  if (!d || !e.start) throw Error("Missing reviewed regional history");
  const points = Array.from({ length: 15 }, (_, i) => { const date = new Date(Date.parse(e.start) + (i - 7) * 86400000).toISOString().slice(0, 10), p = d.points.find(p => p.date === date); return { date, value: p?.quality === "complete" ? p.value : null }; });
  if (points.some(p => p.value === null)) throw Error("Incomplete festival window");
  e.visits = { ...editions[0].visits, regionCode: d.region.code, snapshotId: d.snapshotId, collectedAt: d.collectedAt, points, source: { title: `한국관광공사 ${d.region.name} 일별 외지인 방문`, url: d.source, checkedAt: d.collectedAt, publishedAt: null, sha256: historyHash, note: `2023~2025년 월별 전체 페이지를 확인한 보관본. 원문 지역코드 ${d.sourceRegionCodes.join("/")}. 당시 공개본·축제 입장객·축제 효과가 아님` } };
  e.missing = e.missing.filter(m => m !== "연속 일별 방문 이력");
}
const cost = read("docs/research/evidence/2026-09-10-gongju-cost.json"), gongju = editions.find(e => e.id === cost.editionId);
if (!gongju || gongju.start !== cost.start || gongju.end !== cost.end || cost.expenseParts.reduce((sum, p) => sum + p.amount, 0) !== cost.totalCost) throw Error("Reviewed cost contract mismatch");
gongju.costs = [["budget", "공주 백제문화제 예산액", "결산 공시 예산", cost.budget], ["executed", "공주 백제문화제 집행액", "결산 공시 집행", cost.executed], ["total-cost", "공주 백제문화제 총원가", "원가회계 총원가", cost.totalCost]].map(([id, label, stage, amount]) => ({ id: `gongju-2024-${id}`, label, stage, amount: amount * cost.multiplier, unit: "KRW", year: cost.year, scopeId: "gongju-2024-baekje", scope: "공주시 백제문화제 사업 · 부여 합계 아님 · 예산/집행/총원가 별도 정의", department: "공주시 관광과", vat: "미확인", source: cost.source, compositionKind: id === "total-cost" ? "expense" : "funding", parts: id === "total-cost" ? cost.expenseParts.map(p => ({ label: p.label, amount: p.amount * cost.multiplier })) : null, complete: id === "total-cost" }));
gongju.missing = gongju.missing.filter(m => m !== "공통 기준의 비용 수치").concat(["부가세 포함 기준", "집행액과 총원가 차이의 구체적 원인"]);
writeFileSync(new URL("../data/festival-editions.json", import.meta.url), JSON.stringify({ version: 1, reviewedAt: "2026-09-10", note: "선정·공개 사례 조사에서 연결한 8회차. 논산·공주·임실 방문 이력과 공주 원가회계 추가. 전국 과거 전수 목록이 아니며 현재 API로 덮어쓰지 않음", editions }, null, 2) + "\n");
console.log(`Built ${editions.length} reviewed editions with ${editions.reduce((n,e) => n + (e.visits?.points.length ?? 0), 0)} observed values`);
