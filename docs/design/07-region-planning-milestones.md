...
사용자가 v3 검토 후 M1 개발 진행을 승인했다. 개발 순서는 **M1 → M2 → M3**이다. M4 예산·준비 규모의 첫 구현과 자동 검증을 추가했다. M5 기획안 보관·사업설명/준비 목록 출력을 구현·검증했다. 다음 M6는 결과와 다음 회차에 연결한다.
...
두 후보와 선택 이유가 있는 기획안을 같은 버전으로 보관·출력한다. [사용 흐름](../sdlc/2-design/proposal-implementation.md)과 [검증·게시 상태](../validation/24-planning-proposal.md)를 따른다. M6 결과·다음 회차 연결과 실제 담당자 관찰은 아직 완료하지 않았다.
