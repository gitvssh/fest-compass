...
| 운영안·총예산·결정·현장·실측·보고서·다음 회차 | 개인 기록 기능 사용 가능, 기획 근거 연결은 개발 필요 | `/workspace`는 브라우저 저장. 항목별 예산은 `/planning/budget`에 별도 연결. 기존 운영 기록과 완성 기획안·공동 승인 연결은 후속 |
...
사용자가 v3 검토 후 M1 개발 진행을 승인했다. 개발 순서는 **M1 → M2 → M3**이다. M4 예산·준비 규모의 첫 구현과 자동 검증을 추가했다. M5~M6는 이미 만든 개인 작업을 활용해 연결한다.
