...
기준일: 2026-09-10. 사용자가 주 타겟·정보 수요·최종 의사결정을 명확히 한 내용을 반영한다.
이 문서는 앞으로의 제품 우선순위와 완료 기준의 정본이다. 기존 설계·검증 문서는 구현 이력과 근거로 유지한다.
M1~M6 첫 흐름을 공개했다. 다음 개선은 사용자가 지적한 지도 품질이며, [지도 설계](../sdlc/2-design/region-map-improvement.md)와 [검증·게시 상태](../validation/26-region-map.md)를 따른다. 실제 담당자의 전체 업무 인수는 별도다.
2026-09-09 M1 보완으로 [기준일이 있는 행정경계 지도](../sdlc/2-design/region-boundary-layer.md)를 구현했다. 공식 연계표·도형 검증을 거쳐 시도 15개와 시군구 조회 단위 234개에 2025년 참고 경계를 제공하고, 개편 35개는 목록 조회를 유지한다. [검증·게시 상태](../validation/28-boundary-layer.md)를 따른다. 다음은 실무 과제 관찰과 최신 경계·전국 과거 자료 확대다.

2026-09-10 M2 보완: [현재 등록 행사 기간 겹침](../sdlc/2-design/current-festival-overlap.md)을 구현했다. 기획 기간 전에 시작한 정상 응답을 거부하던 오류를 바로잡고 원래 일정·겹친 구간·당시 검색 방식의 근거 보관을 연결했다. [29번 검증·게시 상태](../validation/29-current-festival-overlap.md)를 따른다.

2026-09-10 M2 거리 보완: [기준 행사에서 가까운 축제 비교](../sdlc/2-design/festival-distance-comparison.md)는 조회한 현재 행사를 등록 좌표의 직선거리순·반경으로 좁힌다. 좌표 미확인 자료는 유지하고 당시 기준·반경·출처를 근거에 보관한다. 전국 자동 반경 검색·거리 기반 이동시간은 후속이다. [30번 검증](../validation/30-festival-distance.md)을 따른다.
...
| 축제 이름 검색·동기간 행사·연관 관광지 조회 | 현재 등록 행사·기간 겹침·조회 자료 내 거리 비교, 전국·연관 관광 확장 후속 | `/compare`에서 지역 최대 3곳의 기간 겹침/기간 안 시작 선택과 일정 그림. `/regions`에서도 계속되는 행사 조회. 조회 결과 내 직선거리순·반경 비교. 전국 전수·주변 지역 자동 추천은 후속 |
| 내 지자체 선택·관광지도·지도와 차트의 연동 | 첫 기능 구현·검증 | `/regions` 전국→시도→시군구·2025년 참고 경계·현재 관광자원/행사·논산 방문 이력·개인 근거 보관. 개편 지역의 최신 경계와 전국 방문 이력은 후속 |
...
| 운영안·총예산·결정·현장·실측·보고서·다음 회차 | 개인 기획·결과 연결 사용 가능 | `/planning/budget`·`/planning/proposal`·`/planning/outcomes`로 예산→기획안→결과→다음 회차를 연결. `/workspace`의 기존 운영 기록과 공동 승인은 별도 |

2026-09-09 현재 **전국 조회→주변/과거 비교→개인 근거→후보·예산→기획안→결과·다음 회차의 첫 흐름**을 사용할 수 있다.
...
| M6 | 운영 결과와 다음 회차 재사용 | 실제 운영·성과·비용을 계획과 비교하고 다음 기획에 활용 | [M6 공개 사용·자동 검증 완료](../validation/25-planning-outcomes.md), 전국 집행 자료·실무 검증 확장 필요 | 원래 계획과 정의가 같은 실측을 비교. 회차별 결과를 M2의 과거 사례로 다시 찾고 M3의 새 기획 근거로 사용 |
...
## M1·M2 지역 방문·공개 비용 보강 (2026-09-10)

공주·임실 2023~2025년 각 1,096일과 공주 백제문화제 2024 원가회계를 연결한다. [화면·자료 계약](../sdlc/2-design/regional-history-costs.md)과 [검증·게시 상태](../validation/32-regional-history-costs.md)를 따른다. 전국 자료 완비와 새 지역 예측 모델을 완료로 표시하지 않는다.
