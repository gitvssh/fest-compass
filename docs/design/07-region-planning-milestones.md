...
이번 변경은 목표·마일스톤 정리이며 아래 미개발 기능을 구현하거나 공개했다는 뜻이 아니다.
...
| 축제 이름 검색·동기간 행사·연관 관광지 조회 | 일부 기반 있음, 공개 탐색 기능은 개발 필요 | 기존 `festivals/new`는 편집 모드 전용. 근거 새로고침의 조회 기능은 지자체 비교검색 화면에 연결되지 않음 |
| 내 지자체 선택·관광지도·지도와 차트의 연동 | 개발 필요 | 현재 라우트·컴포넌트에 지도 화면 없음. 축제 좌표 필드 보유만으로 지도 구현으로 간주하지 않음 |
| 주변·과거 축제의 조건 검색과 나란히 비교 | 개발 필요 | 논산의 고정 연구 사례와 다른 지역·회차를 찾아 비교하는 제품 기능은 다름 |
| 데이터에서 축제 아이템·장소·시기를 도출하고 근거로 담기 | 개발 필요 | 현재 개인 작업은 사용자가 정한 축제명·장소와 운영 가정을 입력하는 흐름 |
| 운영안·총예산·결정·현장·실측·보고서·다음 회차 | 개인 기록 기능 사용 가능, 기획 근거 연결은 개발 필요 | `/workspace`는 브라우저 저장. 항목별 예산 산출·지역 자료 연결·공동 승인 미구현 |

따라서 2026-09-08까지 공개한 것은 **운영 계획과 기록을 시험하는 개인 작업 흐름**이다.
...
|---|---|---|---|---|
| M1 | 내 지자체 관광지도와 현황 | 시군구를 선택하고 관광자원·등록 축제·지역 방문 추세를 지도·차트로 탐색 | 개발 필요, 조회 기반 일부 있음 | 선택 지역·기간이 지도/목록/차트에 함께 적용됨. 항목 선택으로 상세·출처·기준일·자료 단위를 확인하고 근거로 담을 수 있음 |
| M2 | 주변·현재·과거 축제 비교검색 | 주변 지역과 과거 회차를 기간·거리 또는 지역·주제 등의 조건으로 찾고 비교 | 개발 필요 | 비교할 축제와 회차를 구분하고 같은 조건으로 나란히 조회. 일정 중복·지역 연계 자원·자료 누락을 확인하고 검색 조건과 근거를 보관 |
| M3 | 축제 아이템·장소·시기 기획 | 지역 강점과 비교 사례로 주제·아이템·대상·장소·일정 후보를 구성 | 개발 필요 | 적어도 두 기획 대안에 선택한 지도 항목·지표·사례를 연결. 각 후보를 선택하거나 제외한 이유를 근거와 함께 설명 |
| M4 | 예산과 준비 규모 비교 | 프로그램·공간·시설·인력·교통·홍보 등 준비 내역과 비용을 비교 | 총액·자원 입력 사용 가능, 산출 기능 개발 필요 | 수량×단가·출처·가정·제약을 추적하고 항목 합계와 총액이 일치. 서로 다른 규모의 안을 같은 기준으로 비교하고 예산 한도 초과를 표시 |
| M5 | 근거가 연결된 기획안과 결정 | 왜 이 축제·아이템·장소·예산인지 설명하는 기획안을 보관·출력 | 결정 기록·보고서 기반 있음, 기획안 연결 개발 필요 | 조회 조건과 선택 당시 자료·대안·비용·판단 이유가 기획안에 남음. 이후 자료가 갱신돼도 결정 당시 근거를 재확인 가능 |
| M6 | 운영 결과와 다음 회차 재사용 | 실제 운영·성과·비용을 계획과 비교하고 다음 기획에 활용 | 개인 현장·결과·복사 사용 가능, 축적·비교 확장 필요 | 원래 계획과 정의가 같은 실측을 비교. 회차별 결과를 M2의 과거 사례로 다시 찾고 M3의 새 기획 근거로 사용 |
...
검수 후 개발 순서는 **M1 → M2 → M3**이다. M4~M6는 이미 만든 개인 작업을 활용해 연결한다.
...
첫 화면은 남한 전국지도이며 시도→시군구로 좁혀 간다. 우리 지역은 명시적 바로가기로 제공한다. 지역·기간·자료 종류를 바꾸면 지도와 요약 차트가 함께 바뀌고,
...
- M1: 남한 전국→시도→시군구와 상위 복귀, 자료 확보 범위·첫 페이지 제한·지역 코드/경계 검증, 지도/목록 대체를 완료 조건에 포함한다.
