...
상태: 설계·구현·실제 자료를 연결한 로컬 검수 완료, 공개 배포 대기. [검수 기록42](../validation/42-resource-experience.md)를 따른다. 설계19의 데이터·주소·상태 계약을 유지한다.
...
- 선택된 유형·목록/지도 전환의 글자는 더 짙은 파랑(#164ea1)을 쓴다. 실제 계산한 유형 글자 대비는 약 6.45:1, 건수 배경 위에서는 약 5.77:1이다.
...
9. 화면 폭·브라우저 확대·목록/지도 전환으로 지도의 지리적 중심을 바꾸지 않는다. 숨긴 지도의 크기0을 반영하지 않고, 표시될 때 중심을 보존하며 크기를 갱신한다.
