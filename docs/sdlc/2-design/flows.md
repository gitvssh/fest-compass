...
version: v8
summary: "두 목적의 검색·회차·지역·관측연월·달력 연결과 조건 복귀·조회 실패를 구체화합니다. 기존 축제 흐름의 구현·검증은 기록34를 따르고 새 축제 전용 흐름은 구현 코드 있음 · 통합 검증 전입니다."
---
...
새 축제 전용 흐름은 `/new` → `/new/{지역코드}/resources·visits·timing`으로 구현 코드 있음 · 통합 검증 전이며 결과는 [검증 기록35](../../validation/35-pickdday-new-festival.md)를 따른다.
...
[TS-FC-010](../3-testing/scenarios/TS-FC-010.md)과 검증 기록35를 따르며 통합 검증 전이다.
...
새 축제는 작업 브랜치 기준 `/new`(구현 코드 있음 · 통합 검증 전)로 진입한다. 아래 기록 도구는 보조 진입으로 남아 있다.
