...
2026-09-26 관광자원 보강 흐름: 축제/지역 선택 → 자원 유형 선택 → 목록/지도 → 장소 상세·소개.
기존 기본 관광지·문화시설을 유지하고 음식점·숙박을 같은 위계에서 추가한다. 상세·기준점·새 축제의 함께 보기는 별도 행동이다.
[설계19](../../design/19-tourism-resources.md)에 조건 복원·실패·복귀 계약을 둔다. 구현·공개 검증을 완료했다([기록41](../../validation/41-tourism-resources.md)).
...
위는 2025년 한 회차의 흐름 기록이다. 2023·2024년을 포함한 회차 선택은 아래 흐름을 따른다.
...
```mermaid
flowchart TD
  Pick["SCR-FC-008 기존 회차 선택 · 적용"] --> Count{"적용 회차 중 확인된 방문자 특성"}
  Count -->|없음| None["방문자 특성 영역 없음 · 다른 방문 정보는 그대로"]
  Count -->|하나| Single["그 연도의 한 회차 표시 · 기존과 동일"]
  Count -->|둘| Compare["방문자 특성 · 이전 해와 최근 해"]
  Count -->|셋| Compare3["최근 두 회차를 비교 · 세 회차 선택은 유지"]
  Compare3 --> Compare
  Compare --> Age["성·연령 두 해 막대 · 수치 표의 %p"]
  Compare --> Rank["목적지 검색순위 · 두 해 순위와 변화"]
  Rank -->|구분 전환| Rank
  Rank -->|확인된 장소| Resource["SCR-FC-009 해당 관광자원 상세"]
  Resource -->|뒤로가기| Rank
  Pick -->|회차 변경| Count
```

회차를 바꾸면 새 응답의 적용 회차로만 다시 정한다. 늦게 도착한 이전 응답이나 선택하지 않은 회차의 값은 남기지 않는다.
관광자원에서 돌아오면 적용 회차·차트 기간·목적지 구분과 누른 장소의 초점을 유지한다. 방문 구성 두 회차 요약도 같은 적용 회차를 따른다.
기록·저장은 요구하지 않는다. 세 회차가 모두 적용되면 선택은 그대로 두고 확인된 최근 두 회차를 비교하며, 방문 구성은 세 행을 유지한 채
요약만 최근 두 회차로 계산한다. 다른 두 회차는 위쪽 회차 선택에서 고른다([기능 상세 §11 표 2·5](functional-spec.md#11-두-회차-방문자-특성-비교)).
구현·실행 결과는 [기록39](../../validation/39-edition-profile-comparison.md), 인수 기준은 [TS-FC-014](../3-testing/scenarios/TS-FC-014.md).
