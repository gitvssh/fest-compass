...
2026-09-26 공통 분석: 검색한 등록 축제마다 `과거 방문 흐름 / 주변 관광자원 / 개최 시기`의 세 메뉴를 동일하게 제공한다.
방문 본문은 시군구 월별·일별 자료가 기본이며 검토된 실제 회차가 있으면 같은 화면에서 비교한다. 현재 등록·보관 기록은
확인된 동일 축제만 한 결과로 합친다. 자료가 더 있는 축제 때문에 별도 제품·지원 목록·필수 준비 단계를 만들지 않는다.
[화면 목표18](../../design/18-unified-festival-analysis.md)·[검증40](../../validation/40-unified-festival-analysis.md)을 따른다.
...
[기능 상세 §8](functional-spec.md#8-관련-자료-검색원문-연결)·[검증 기록36](../../validation/36-related-material-search.md)을 따른다.
