...
[공개 비교 화면 열기](https://pickday.damecasol.com/compare) · [담은 비교 근거](https://pickday.damecasol.com/evidence#comparisons)
...
첫 M2 구현 뒤 도로·참고 경계 지도가 추가됐다. [현행 화면](../../review/2026-09-current-state.md)에서 현재 연결을 확인한다.

2026-09-10 후속: [현재 행사 기간 겹침](current-festival-overlap.md)으로 날짜 조회 가정을 바로잡았다. 아래의 당시 시작일 검색 제한은 후속 설계로 대체한다. 기획 후보·예산·기획안 연결도 현재 M3~M6에서 제공하며, 아래 미제공 문구는 첫 구현 시점의 이력이다.

## 사용할 수 있는 행동

1. 과거 회차를 지역·겹치는 기간·이름·주제로 찾는다. 개최일이 없으면 사업연도 조건으로 남긴다.
2. 현재 등록 행사는 직접 선택한 지역 1~3곳과 기획 기간으로 조회한다. 기간 겹침/기간 안 시작을 고르고 지역별 전체 페이지·건수·조회 시각과 실패를 확인한다.
...
공동 저장·공식 승인은 아직 제공하지 않는다. 비교 근거에서 후보 기획으로 이동하고 선택 당시 사본을 연결하는 기능은 제공한다.

## 실제 연결 자료

2026-09-22 코드·보관본 기준이다. 공주·임실 자료 확대의 원문과 당시 검증은 [지역 이력·비용 확대](regional-history-costs.md)를 따른다.
...
| 임실N치즈축제 2023·2024·2025 | 연도별 제전위원회 홈페이지. 일수 4·4·5일 | 시군구 일별 외지인 방문 D-7~D+7 각 15일 연결. 공통 비용 미확보 |
| 백제문화제 공주 2024 | 공주시 결산 재정공시 PDF 97쪽(인쇄 91쪽), 9/28~10/6 | 시군구 일별 외지인 방문 15일과 공개 예산·집행·총원가 및 원가 구성 연결. 부여와 합치지 않음 |
...
- **현재 조회:** 기존 `/api/regions`의 검증·캐시·호출 제한을 재사용한다. 기간이 겹치는 정상 응답을 허용하고 시작일 검색은 결과를 좁히는 선택으로 제공한다. 전국 전수나 실제 개최·취소의 완전성은 보증하지 않는다. 현재 상세로 과거 회차를 덮지 않는다.
...
실제 담당자 관찰과 전국 자료 확보는 후속이다. 후보·예산·기획안 연결은 후속 M3~M6에 구현됐으며 [현행 범위](../../review/2026-09-current-state.md)를 따른다.
