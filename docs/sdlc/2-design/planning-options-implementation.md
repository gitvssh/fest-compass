...
M3 첫 구현의 저장 키는 `fest-compass.planning.v1`, 파일은 `fest-compass-planning` 형식 v1이었다.
현재 M4는 같은 키에 파일 v2를 저장하며 v1 읽기와 기존 보관본 보존을 지원한다. [현재 호환 계약](budget-implementation.md)을 따른다.
...
예산 수량×단가 산출과 재원/지출·규모 비교는 [M4](budget-implementation.md)에 연결했다. 완성 기획안·사업설명/준비 목록 출력은 M5에서 연결한다.
