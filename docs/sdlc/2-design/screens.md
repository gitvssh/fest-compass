...
version: v8
summary: "화면 목표·정보 순서·기본 배치를 디자인 작업에 전달합니다. v5·v6·v7 그림은 개발 전 설계이며 기존 축제의 실제 화면·실행 검증은 기록34를 따릅니다. 최종 시각 디자인은 사용자 작업입니다."
---
...
기존 축제의 실제 구현과 검증은 [기록34](../../validation/34-existing-festival-journey.md)를 따른다. 새 축제 전용 흐름은 개발 필요다. 아래 v7 그림은 기존 화면의 부족한 자료·오류 상태를 설명하는 개발 전 기본 배치다.
...
SCR-FC-008~010과 SCR-FC-011의 목적 선택·기존 축제 검색을 구현했다. SCR-FC-012~014와 새 축제 전용 지역 선택은 **개발 필요**다. 새 축제의 대상 관광객·아이템 결정과 기획안 작성은 탐색의 선행 조건이 아니다.
...
| SCR-FC-011 | 목적 선택·축제 검색·새 축제 지역 선택 | `/`, `/existing/search`; `/new`는 제안 | 부분 구현 · 새 축제는 `/regions` 연결 | 목적 선택·기존 축제 검색 또는 새 축제의 시도/시군구 선택 |
...
목적을 바꿀 수 있다. 기존 축제는 검증 기록34의 구현 범위를 따르며 새 축제 전용 흐름은 계획이다.
