...
version: v7
summary: "화면 목표·정보 순서·기본 배치를 디자인 작업에 전달합니다. 정상 상태 v5·v6와 부족한 자료·실패 상태 v7은 미구현 설계이며 최종 시각 디자인은 사용자 작업입니다."
---
...
앱 구현은 개발 필요다. 아래 v7은 새 화면 ID를 만들지 않고 기존 화면의 부족한 자료·오류 상태를 보완한다.
...
새 SCR-FC-008~014의 배치·경로는 **미구현 계획**이다. 새 축제의 대상 관광객·아이템 결정과 기획안 작성은 탐색의 선행 조건이 아니다.
...
| SCR-FC-008 | 기존 축제 과거 방문 흐름 | `/existing/{festivalId}/visits` (제안) | planned · 미구현 | 지역 일별 방문 추이·회차별 같은 Y축·실제 날짜/요일·행사 음영·수치 표 |
| SCR-FC-009 | 기존 축제 주변 관광자원 | `/existing/{festivalId}/resources` (제안) | planned · 미구현 | 지역 전체 지도·목록·관광자원 상세·명시적 지도 기준점 조회 |
| SCR-FC-010 | 기존 축제 개최 시기 비교 | `/existing/{festivalId}/timing` (제안) | planned · 미구현 | 지역 월별 방문 흐름·후보 기간 달력·연결된 공휴일/행사 |
| SCR-FC-011 | 목적 선택·축제 검색·새 축제 지역 선택 | `/` 개편 및 `/existing`, `/new` (제안) | planned · 미구현 | 목적 선택·기존 축제 검색 또는 새 축제의 시도/시군구 선택 |
| SCR-FC-012 | 지역 관광자원 | `/new/{regionKey}/resources` (제안) | planned · 미구현 | 새 축제의 첫 탐색·지역 전체 지도/목록·자원 상세·최대 2개 함께 보기 |
| SCR-FC-013 | 지역 방문 흐름 | `/new/{regionKey}/visits` (제안) | planned · 미구현 | 지역 전체 월별·요일별 일평균·관측 기간·수치 표 |
| SCR-FC-014 | 개최 시기 | `/new/{regionKey}/timing` (제안) | planned · 미구현 | 연월 달력 우선·연결된 공휴일/행사·선택적 최대 2기간 비교 |

새 제안 경로는 현재 라우터나 API 계약에 존재한다는 뜻이 아니다. 기존 001~007의 ID·설명·v3 자산은
...
확인된 0과 미조회·누락을 구분한다. AI 테마 추천·점수·뉴스를 이 흐름에 추가하지 않는다.
...
그대로 재사용하지 않는다. 동일 날짜 원자료로 새 구간을 구성하는 일은 미구현 가공 항목이다.
...
목적을 바꿀 수 있다. 두 목적 모두 구현 계획이며 앱의 현재 기능과 혼동하지 않는다.
