...
version: v12
summary: "화면 목표·정보 순서·기본 배치를 디자인 작업에 전달합니다. v5·v6·v7 그림은 개발 전 설계이며 기존 축제의 실제 화면·실행 검증은 기록34를 따르고 새 축제 화면은 사용 가능입니다. 임실 두 회차 방문자 특성 비교는 개발 중입니다. 최종 시각 디자인은 사용자 작업입니다."
---
...
상태: **개발 중 — 자료 확인 완료, 구현·공개 검증 대기.** SCR-FC-008의 기존 방문자 특성 자리에서, 적용한 두 회차의
성·연령 비율과 목적지 검색순위를 오래된 해 → 최근 해 순서로 나란히 읽는다. 한 회차만 적용하면 기존 한 회차 표시를 그대로 쓴다.
세 회차를 적용하면 확인된 최근 두 회차를 비교하고 제목에 그 두 연도를 쓴다. 제목 옆 짧은 `최근 두 회차` 표시는 선택 사항이다.
