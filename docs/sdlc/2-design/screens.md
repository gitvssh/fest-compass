...
version: v13
summary: "화면 목표·정보 순서·기본 배치를 디자인 작업에 전달합니다. v5·v6·v7 그림은 개발 전 설계이며 기존 축제의 실제 화면·실행 검증은 기록34를 따르고 새 축제 화면은 사용 가능입니다. 임실 두 회차 방문자 특성 비교는 사용 가능합니다. 최종 시각 디자인은 사용자 작업입니다."
---

# 화면설계서

2026-09-26 관광자원 보강: 기존·새 축제에 같은 네 유형 선택과 공통 소개를 제공한다.
[설계19](../../design/19-tourism-resources.md)의 화면 목표·기본 배치가 SCR-FC-009/012의 최신 보강안이다.
최종 시각 디자인은 사용자 작업이며 구현·공개 검증은 [기록41](../../validation/41-tourism-resources.md)에서 확인한다. 이전 그림은 당시 참고 배치로 유지한다.

## 고른 모든 등록 축제의 공통 분석 · 2026-09-26

SCR-FC-011 검색은 검토된 동일 축제의 현재 등록·보관 기록을 한 카드로 보여준다. SCR-FC-008은 모든 등록 축제의
시군구 월별·일별 방문을 기본으로 제공하고, 검토된 과거 회차가 있으면 같은 이름·메뉴 안에서 회차 비교를 연다.
SCR-FC-009 관광자원·010 개최 시기는 같은 지역을 유지한다. 새 축제 SCR-FC-013도 같은 전국 관측 자료를 사용한다.

화면 목표는 고른 지역의 방문 계절성을 읽고 관광자원·일정 검토로 이동하는 것이다. 축제명·지역 → 세 메뉴 → 지역 방문 제목 →
등록 일정 바로가기(있을 때) → 관측연도·월 차트 → 선택 월의 일별 차트·수치 표 순서다. 없는 보조 지표의 빈 카드는 만들지 않는다.
[기본 배치18](../../design/18-unified-festival-analysis.md)과 [실행·공개 상태40](../../validation/40-unified-festival-analysis.md)를 따른다.
아래 v5~v7의 ‘현재 등록만 있으면 과거 기록 없음’ 그림은 개발 전 이력이며 현재 동작으로 쓰지 않는다.

## 임실 두 회차 방문자 특성 비교 · 사용 가능

상태: **사용 가능 — 원자료·구현·공개 검증 완료.** SCR-FC-008의 기존 방문자 특성 자리에서, 적용한 두 회차의
성·연령 비율과 목적지 검색순위를 오래된 해 → 최근 해 순서로 나란히 읽는다. 한 회차만 적용하면 기존 한 회차 표시를 그대로 쓴다.
세 회차를 적용하면 확인된 최근 두 회차를 비교하고 제목에 그 두 연도를 쓴다. 제목 옆에 `최근 두 회차`와 기존 회차 선택으로 돌아가는 `비교할 회차 바꾸기` 링크를 둔다.

| 영역 | 사용자가 이루려는 일 | 읽기 순서 | 다음 행동 |
|---|---|---|---|
| 방문 구성 두 회차 요약 | 두 해의 개최 읍·면·동 방문 규모와 외지인 비율 차이 파악 | 기존 회차 막대 → 하루 평균 변화 → 외지인 비율 %p | 기간 합계 표·출처·모든 개최연도 |
| 성·연령 비교 | 어떤 연령·성별의 비중이 달라졌는지 읽기 | 제목(두 연도) → 두 회차 날짜·일수·성수면 → 연령대별 두 해 막대와 값 | 수치 표(두 해·%p)·출처 |
| 목적지 순위 비교 | 함께 검색된 장소의 순위 변화로 관광 연계 후보 살피기 | 구분 전환 → 장소·두 해 순위·변화 문구 | 확인된 장소의 `관광자원에서 보기` |

모든 막대에 연도 글자를 붙이고 두 해를 같은 축에 둔다. 변화는 `2위 상승`·`1위 하락`·`같음`·`—`, `+0.2%p`처럼 문자로 읽는다.
새 메뉴·선택기·설명 배너는 추가하지 않는다. 기본 와이어프레임과 다섯 개 표시 규칙은
[기능 상세 §11](functional-spec.md#11-두-회차-방문자-특성-비교), 실행·공개 결과는
[기록39](../../validation/39-edition-profile-comparison.md)를 따른다. 최종 시각 디자인은 사용자 작업이다.
...
이 절은 2025년 한 회차 기록이다. 2023·2024년 연결과 두 회차 비교의 표시 조건은 맨 위 절과 [기능 상세 §11](functional-spec.md#11-두-회차-방문자-특성-비교)이 대체한다.
