...
version: v8
summary: "화면 목표·정보 순서·기본 배치를 디자인 작업에 전달합니다. v5·v6·v7 그림은 개발 전 설계이며 기존 축제의 실제 화면·실행 검증은 기록34를 따르고 새 축제 화면은 구현 코드 있음 · 통합 검증 전입니다. 최종 시각 디자인은 사용자 작업입니다."
---
...
기존 축제의 실제 구현과 검증은 [기록34](../../validation/34-existing-festival-journey.md)를 따른다. 새 축제 전용 흐름은 구현 코드 있음 · 통합 검증 전이며 결과는 [기록35](../../validation/35-pickdday-new-festival.md)를 따른다. 아래 v7 그림은 부족한 자료·오류 상태를 설명하는 개발 전 기본 배치다.
...
SCR-FC-008~010과 SCR-FC-011의 목적 선택·기존 축제 검색을 구현했다. SCR-FC-012~014와 새 축제 전용 지역 선택은 **구현 코드 있음 · 통합 검증 전**이다. 새 축제의 대상 관광객·아이템 결정과 기획안 작성은 탐색의 선행 조건이 아니다.
...
| SCR-FC-011 | 목적 선택·축제 검색·새 축제 지역 선택 | `/`, `/existing/search`, `/new` | 목적 선택·기존 축제 검색은 구현 · 검증 기록34. `/new` 지역 선택은 구현 코드 있음 · 통합 검증 전(공개 서비스는 아직 `/regions` 연결) | 목적 선택·기존 축제 검색 또는 새 축제의 시도/시군구 선택 |
| SCR-FC-012 | 지역 관광자원 | `/new/{지역코드}/resources` | 구현 코드 있음 · 통합 검증 전 | 새 축제의 첫 탐색·지역 전체 지도/목록·자원 상세·최대 2개 함께 보기 |
| SCR-FC-013 | 지역 방문 흐름 | `/new/{지역코드}/visits` | 구현 코드 있음 · 통합 검증 전 | 지역 전체 월별·요일별 일평균·관측 기간·수치 표 |
| SCR-FC-014 | 개최 시기 | `/new/{지역코드}/timing` | 구현 코드 있음 · 통합 검증 전 | 연월 달력 우선·연결된 공휴일/행사·선택적 최대 2기간 비교 |

`/new` 경로는 작업 브랜치 코드에 있으나 통합 검증·공개 반영 전이다. 지역코드는 5자리 행정 코드이며 세종 `/new/36110`은
...
현재 구현에서는 첫 화면의 `지역부터 살펴보기`가 별도 주소 `/new`의 지역 선택으로 이동한다(구현 코드 있음 · 통합 검증 전).
