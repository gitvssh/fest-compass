...
지도와 목록으로 확인하고, 논산·공주·임실의 확보된 방문 추세를 선그래프·수치 표로 조회해 필요한 자료를 담을 수 있다.
실제 서비스의 [전국 관광지도](https://pickday.damecasol.com/regions)와 [담은 근거](https://pickday.damecasol.com/evidence)를 사용한다.
공개 반영·검증 결과는 [M1 검증 기록](../../validation/20-region-explorer.md)에 기록한다.

2026-09-22 후속 구현을 반영했다. 아래 캡처와 개별 실호출 건수는 9월 8일 첫 구현 당시 증거다.
현재 전체 범위는 [현행 점검](../../review/2026-09-current-state.md), 자료별 보유·갱신은 [데이터 현황](../../research/2026-09-data-inventory.md)을 따른다.
...
현재는 [도로 배경·간단 지도 전환](region-map-improvement.md)과 [기준일이 있는 참고 경계](region-boundary-layer.md)를 제공한다.
간단 지도는 Natural Earth 5.1.2 대한민국 육지 윤곽을 사용한다. 2025년 경계는 234개 조회 단위에 연결하고 개편 35개는 목록으로 탐색한다.
참고 경계의 선택과 현재 법정 영역 인증·필지 조회·면적별 통계 배분은 구분한다. 좌표 자원은 지도와 목록에 표시한다.
...
| 공주·임실 방문 추세 | 각 지역의 2023~2025년 1,096일씩 추가 보관·연결 | [자료 확대](regional-history-costs.md). 그 밖의 미확보 지역·기간에는 다른 지역 값을 대신 표시하지 않음 |
...
관광지·문화시설은 현재 등록 목록이다. 행사 목록은 선택 기간과 **겹치는** 현재 등록 행사를 조회한다.
[기간 겹침 검증](current-festival-overlap.md)이 첫 구현 당시의 시작일 검색 설명을 대체한다.
비교 화면에서는 기간 안 시작 조건도 선택할 수 있다. 빈 결과는 실제 행사 부재의 증거가 아니다.
...
담은 근거에서 기획 후보로 이동하고 당시 사본을 연결하는 기능은 M3~M5에 구현됐다.
관광지·문화시설의 이름·주소를 후보 장소에 적용하는 범위는 [장소 입력 연결](venue-evidence.md)을 따른다.
...
- 공주·임실에서는 해당 지역의 보관 이력이 보이고, 미확보 지역·기간에서는 누락을 실제 0명으로 오해하지 않는가.
